...
pub const WAVE_K:     f64 = 0.8;   // fixed wave vector — radians per geodesic unit
pub const SIGMA_INIT: f64 = 0.5;   // initial wave packet width
pub const SIGMA_MIN:  f64 = 0.05;  // minimum width — Zeno floor
pub const SIGMA_MAX:  f64 = 0.8;   // maximum width
...
pub fn personality_phase(personality: &str) -> f64 {
    match personality {
        ...
        "Eros"           => 3.0 * PI / 4.0,
        "UnifiedOmniAGI" => PI,
        _                => PI / 2.0,  // default to Gaia phase
    }
}
...
pub struct ConceptPoint {
    ...
    pub sigma:        f64,   // wave packet width — Zeno consolidation
    pub phase:        f64,   // personality phase — interference term
}
...
impl ConceptPoint {
    pub fn new(
        ...
        // Vary starting depth slightly by name hash to prevent exact clustering
        let mut hash: u64 = 5381;
        for b in name.bytes() {
            hash = hash.wrapping_mul(33).wrapping_add(b as u64);
        }
        let depth_variation = (hash % 1000) as f64 / 10000.0;  // 0.000 - 0.099
    ...
    /// Approved → drifts toward core, wave packet narrows.
    /// Blocked  → drifts toward frontier, wave packet broadens.
    pub fn update_depth(&mut self, approved: bool) {
        if approved {
        ...
        // Recompute position at new depth
        let pos  = self.position_vec();
        let norm = pos.norm().max(1e-10);
        let new_pos = &pos * (self.target_depth / norm);
...
impl SpatialIndex {
    ...
    ///
    /// The cosine term is the interference factor:
    ///   - Same personality phase → cos near 1 → constructive → high score
    ///   - Orthogonal phase      → cos near 0 → suppressed
    ///   - Opposite phase        → cos near -1 → destructive → floored to 0
    pub fn nearest_with_phase(
        ...
        let mut scored: Vec<(f64, &ConceptPoint)> = self.concepts.iter()
            ...
            .filter_map(|c| {
                ...
                // Floor to zero — no negative scores
                ...
                if score > 1e-10 {
                    Some((-score, c))  // negate for ascending sort = descending score
                } else {
                    None
                }
            })
    ...
    /// Standard nearest — used when no phase context available.
    /// Falls back to geodesic distance only.
    ...
    pub fn consolidate_depth(&mut self, name: &str, approved: bool) {
...
mod tests {
    ...
    fn test_same_phase_higher_score_than_orthogonal() {
        ...
        let pos_orth = ball_vec(1.0, SOUL_DIM);  // same position, different phase

        let mut c_same = ConceptPoint::new("same", &pos_same, 1.0, "Khaos", 1.2, 1.0, 0);
        c_same.phase = 0.0;  // Khaos phase

        let mut c_orth = ConceptPoint::new("orth", &pos_orth, 1.0, "Gaia", 1.2, 1.0, 0);
        c_orth.phase = PI / 2.0;  // Gaia phase — orthogonal to Khaos

        index.concepts.push(c_same);
        index.concepts.push(c_orth);

        // Query with Khaos phase
        let query_phase = 0.0;
        let results = index.nearest_with_phase(&pos_same, query_phase, 2);

        // Same-phase concept should score higher
        if results.len() >= 1 {
    ...
    fn test_opposite_phase_suppressed() {
        ...
        c_opposite.phase = PI;  // opposite to query phase 0

        index.concepts.push(c_opposite);

        // Query with phase 0 — opposite concept should be suppressed
        let results = index.nearest_with_phase(&pos, 0.0, 5);
        // cos(π + 0 - 0) = cos(π) = -1, floored to 0, so no result
    ...
    fn test_depth_consolidates_on_approval() {
        ...
        index.consolidate_depth("x", true);
    ...
    fn test_depth_drifts_on_block() {
        ...
        c.target_depth = 0.5;
        index.concepts.push(c);
        index.consolidate_depth("x", false);
